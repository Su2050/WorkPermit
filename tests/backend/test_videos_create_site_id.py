...
class TestVideoCreateSiteId:
    """测试视频创建时的site_id字段"""
    
    def test_create_video_without_site_id_should_fail(self, client):
        """测试：创建视频时不提供site_id应该返回422错误"""
        video_data = {
            # 缺少 site_id
            "title": f"测试视频_{int(time.time())}",
    ...
    def test_create_video_with_site_id_should_success(self, client, test_site_id):
        """测试：创建视频时提供site_id应该成功"""
        unique_id = int(time.time())
        video_data = {
            "site_id": test_site_id,  # 提供site_id
            "title": f"测试视频_{unique_id}",
            "file_url": f"https://example.com/video_{unique_id}.mp4",
            "duration_sec": 120,
            "description": "测试描述"
        }
        resp = client.post("/videos", json_data=video_data)
        ...
        assert "video_id" in data.get("data", {}), "响应中缺少video_id"
        
        # 清理
        video_id = data["data"]["video_id"]
        client.session.delete(f"{BASE_URL}/videos/{video_id}")
        print(f"✓ 提供site_id时成功创建视频")
    
    def test_create_video_with_invalid_site_id_should_fail(self, client):
        """测试：创建视频时提供无效的site_id应该失败"""
        unique_id = int(time.time())
        video_data = {
        ...
        # 应该返回错误（403、404或500都是合理的错误）
        assert resp.status_code in [200, 403, 404, 500], f"期望错误，实际状态码: {resp.status_code}"
        if resp.status_code == 200:
            data = resp.json()
